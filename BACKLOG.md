...
- [ ] **1.7** Set up CI (GitHub Actions) to run lint + tests on push/PR
...
- [ ] **2.1** Fix `_handlePaste`: replace global `event` with the `e` parameter on lines 461 and 474
- [ ] **2.2** Fix `simulate()`: dispatch events on `oDomElement` instead of hardcoded `document` (line 213)
- [ ] **2.3** Fix `_handleKeyDown`: guard `onKeyProcess` call so `character` is not `undefined` when a prefix/suffix key was matched (line 448)
- [ ] **2.4** Fix `decodeKeyEvent`: return a string `'0'`–`'9'` for numpad keys instead of a number (line 181)
- [ ] **2.5** Fix `ignoreIfFocusOn`: handle DOM element values (not just CSS selector strings) without crashing on `.matches()` (line 257)
- [ ] **2.6** Clear any pending `testTimer` in `attachTo` / `detachFrom` to prevent stale timers firing after detach
...
- [ ] **3.1** Rewrite `decodeKeyEvent` to use `event.key` as the primary source instead of keyCode ranges
- [ ] **3.2** Filter out only control/modifier keys (`Shift`, `Control`, `Alt`, `Meta`, `Dead`, etc.) and keys with `.key` length > 1 that aren't printable
- [ ] **3.3** Remove `_getNormalizedKeyNum` (uses deprecated `e.which` / `e.keyCode`) or keep only as an internal fallback for `suffixKeyCodes`/`prefixKeyCodes`
- [ ] **3.4** Support hyphens, colons, brackets, and other punctuation that scanners commonly send
- [ ] **3.5** Add unit tests for the new decoder covering: letters, numbers, numpad, shifted characters, punctuation, modifier-only keys, and Android Chrome edge cases
...
- [ ] **4.1** Replace all `var` with `const` / `let`
- [ ] **4.2** Replace `_mergeOptions` with `Object.assign` or object spread
- [ ] **4.3** Replace DOM expando (`oDomElement.scannerDetectionData`) with a `WeakMap` keyed by element
- [ ] **4.4** Convert UMD wrapper to ES module source; let the bundler (Phase 1.3) produce UMD/CJS outputs
- [ ] **4.5** Use arrow functions where `this` binding to the DOM element is not needed
